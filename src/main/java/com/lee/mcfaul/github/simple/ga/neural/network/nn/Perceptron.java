 ...
 * of input will be submitted to a perceptron and it will weight each input based on the weight pre-assigned to it and
 * add the resulting numbers together. if the final tally exceeds the threshold value then the perceptron will "fire"
 * and output a value of 1, otherwise it will output 0. For simplicity all input should be normalized to be between 0
 * and 1 before being submitted at the input level.
 * <p>
 * note that the lombok library is used here to simplify the code.
 */
@Getter @AllArgsConstructor public class Perceptron {

	private static final String BAD_INPUT_LENGTH_MSG = "Input length %d does not match expected layer length %d.";

	/**
	 * Represents the wights given to this perceptron's input. There is one wight per input value so the length of the
	 * array will be equal to the number of perceptrons in the previous layer or the size of the ingest array if this
	 * perceptron is part of the input layer.
	 */
	private double[] weights;

	/**
	 * This threshold value must be exceeded by the result of an input calculation in order for the perceptron to "fire"
	 */
	 ...
	 *
	 * @param previousLayerSize the size of the input layer for this perceptron (previous layer or input)
	...
	/**
	 * Return a new perceptron with randomly selected "genes" (weight and threshold values) from both this perceptron
	 * and the given perceptron
	 *
	...
	/**
	 * Calculate the sum of weighted inputs and return whether the sum exceeds the threshold as a "fire" (1)
	 * or a no "fire" (0)
	 *
